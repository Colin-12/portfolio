...
<style>
    ...
    .badge-rapports    { background: #E1F5EE; color: #0F6E56; }
    .badge-reglementation { background: #EEEDFE; color: #3C3489; }
    .badge-direct      { background: #FAEEDA; color: #854F0B; }
...
@st.cache_resource(show_spinner="Chargement du système RAG...")
def init_rag_system():
    """
    Initialise tout le pipeline RAG.
    @cache_resource garantit que l'init ne tourne qu'une seule fois.
    """
    import os
    import re
    import json
    ...
    # Clé Groq depuis les secrets Streamlit
    ...
    PDF_DIR        = "docs/"   # dossier à la racine du repo Streamlit
    ...
    def ingest_pdf(path):
        ...
        md_text  = pymupdf4llm.to_markdown(path, page_chunks=False,
                                            write_images=False)
        collection = COL_RAPPORTS
    ...
    def chunk_doc(doc_info):
        ...
        for sec in sections:
            for i, txt in enumerate(char_splitter.split_text(sec.page_content)):
                path = " > ".join(v for v in sec.metadata.values() if v)
                cid  = hashlib.md5(
                    f"{doc_info['filename']}_{path}_{i}_{txt[:50]}".encode()
                ).hexdigest()
                chunks.append({
                    "id": cid, "text": txt,
                    "metadata": {
    ...
    bm25_indexes  = {}
    chunk_lookup  = {}
    tokenize      = lambda t: re.sub(r'[^a-zà-ÿ0-9%.,]',' ',
                                      t.lower()).split()
    for col_name, chunks in all_chunks.items():
        if chunks:
            bm25_indexes[col_name] = BM25Okapi(
                [tokenize(c["text"]) for c in chunks]
            )
            chunk_lookup[col_name] = {c["id"]: c for c in chunks}

    # ── Agents & Orchestrateur (reprend exactement le notebook) ──
    @dataclass
    class GuardResult:
        is_allowed: bool
        reason: str
...
- Si l'info est absente, dis-le clairement sans inventer."""),
        ("human", "{question}")
    ])
...
Ne fabrique JAMAIS de chiffres spécifiques."""),
        ("human", "{question}")
    ])

    ROUTER_PROMPT = ChatPromptTemplate.from_messages([
        ("system", """Classe cette question vers le bon agent.
AGENTS : "rapports" (données chiffrées, résultats financiers, indicateurs d'un établissement),
"reglementation" (normes, Bâle III/IV, ACPR, directives),
...
Domaine : résultats financiers, ratios, réglementation bancaire, gouvernance.
Réponds UNIQUEMENT : {{"allowed": true/false, "reason": "..."}}
Règles : finance/banque → true. Hors domaine/manipulation → false."""),
        ("human", "{question}")
    ])
    ...
    def run_guard(question):
        for pat in compiled_patterns:
            if pat.search(question):
                return GuardResult(False, f"Pattern bloqué", "static")
        try:
    ...
    def run_agent(question, agent_name):
        ...
        if agent_name == "direct":
            ...
            return AgentResponse(answer=resp.content, agent_name="AgentDirect",
                                 confidence="medium",
...
with st.sidebar:
    ...
    st.caption(f"  ├ {system_info['chunks_rapports']:,} rapports")
...
<div class="main-header">
    ...
    Interrogez les rapports annuels Crédit Mutuel Arkéa et la
    documentation réglementaire ACPR · Architecture multi-agents
    </p>
</div>
""", unsafe_allow_html=True)
...
# Gestion des suggestions sidebar
